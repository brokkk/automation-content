-- ============================================
-- AI Content Pipeline - Safe Migration Script
-- Run this INSTEAD of schema.sql if you get errors
-- about types/tables already existing
-- ============================================

-- Drop existing types (safe to run multiple times)
DROP TYPE IF EXISTS content_status CASCADE;
DROP TYPE IF EXISTS platform_type CASCADE;
DROP TYPE IF EXISTS llm_provider_type CASCADE;
...
CREATE TABLE image_templates (
  ...
  supported_formats TEXT[] DEFAULT ARRAY['1080x1350'],
  category_id       UUID REFERENCES categories(id) ON DELETE SET NULL,
  is_default        BOOLEAN DEFAULT false,
  created_at        TIMESTAMPTZ DEFAULT NOW(),
  updated_at        TIMESTAMPTZ DEFAULT NOW()
...
CREATE POLICY "Allow anon read rss_sources" ON rss_sources FOR SELECT TO anon USING (true);
CREATE POLICY "Allow anon read personas" ON personas FOR SELECT TO anon USING (true);
CREATE POLICY "Allow anon read content_items" ON content_items FOR SELECT TO anon USING (true);
CREATE POLICY "Allow anon read app_settings" ON app_settings FOR SELECT TO anon USING (true);

-- Anon can write for demo (you may want to restrict this in production)
CREATE POLICY "Allow anon insert content_items" ON content_items FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "Allow anon update content_items" ON content_items FOR UPDATE TO anon USING (true);
CREATE POLICY "Allow anon insert rss_sources" ON rss_sources FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "Allow anon update rss_sources" ON rss_sources FOR UPDATE TO anon USING (true);
CREATE POLICY "Allow anon delete rss_sources" ON rss_sources FOR DELETE TO anon USING (true);
...
CREATE POLICY "Allow anon insert system_events" ON system_events FOR ALL TO anon USING (true);
CREATE POLICY "Allow anon insert approval_logs" ON approval_logs FOR ALL TO anon USING (true);

-- Service role has full access
CREATE POLICY "Service role full access categories" ON categories FOR ALL TO service_role USING (true);
CREATE POLICY "Service role full access content_items" ON content_items FOR ALL TO service_role USING (true);
CREATE POLICY "Service role full access llm_runs" ON llm_runs FOR ALL TO service_role USING (true);
CREATE POLICY "Service role full access approval_logs" ON approval_logs FOR ALL TO service_role USING (true);
